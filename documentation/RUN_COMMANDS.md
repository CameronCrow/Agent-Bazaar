...
Runs the full Exp1 matrix for every model listed in `documentation/open_weights_models.json` via OpenRouter. Edit that JSON to define the sweep (each entry is `{"display_name", "params_b", "slug"}`); the repo ships with a single example entry. Use `--models` to filter the loaded list by substring.
