"""Consumer agents for the B2C scenarios.

``CESConsumerAgent`` is the main consumer used in THE_CRASH and the standard
B2C runs: a CES utility maximiser with labour supply, an income process, and
a per-good willingness-to-pay (WTP / eWTP) machinery driving order placement.
``FixedConsumerAgent`` is a deterministic non-LLM baseline. Five ``consumer_persona_type``
flavours (LOYAL, SMALL_BIZ, PRICE_HAWK, POPULAR, VARIETY) modulate which firm
a consumer prefers within their visible discovery slice.
"""
import logging
import numpy as np
import math
...
class CESConsumerAgent(LLMAgent):
    """LLM-seeded CES consumer with labour supply and willingness-to-pay tracking.

    Demand each timestep follows a CES utility ``U = (sum_g alpha_g * x_g^((sigma-1)/sigma))^(sigma/(sigma-1))``
    with goods weights ``ces_params`` (drawn once via the LLM from the consumer's
    ``persona`` if not supplied at construction). Cost-of-living, demand,
    willingness-to-pay, and an EMA expected-WTP are all recomputed per step
    from observed prices. ``make_orders`` dispatches to one of several scenario
    branches (THE_CRASH, RACE_TO_BOTTOM, RATIONAL_BAZAAR, etc.).
    """
    ...
    def __init__(
        ...
        """Construct a CES consumer with labour skill ``v`` (uniform on [1.24, 159.1] when ``skill=-1``), CES parameters from ``ces_params`` if given else generated by the LLM, and an empty per-good WTP cache."""
    ...
    def compute_labor_disutility(self) -> float:
        """Convex disutility of labour: ``c * l^delta`` with optional ``_labor_disutility_scale`` for daily/weekly timesteps."""
    ...
    def compute_goods_utility(self) -> float:
        """Evaluate the CES sub-utility ``(sum_g alpha_g * q_g^((sigma-1)/sigma))^(sigma/(sigma-1))`` over the current inventory."""
        goods_total = 0.0
    ...
    def compute_cash_utility(self) -> float:
        """Log savings utility ``beta * log(epsilon + cash)``; returns 0 when CRRA savings is enabled."""
...
class FixedConsumerAgent:
    ...
    Buys ``quantity_per_good`` units of each good per step from the first
    available quote (subject to discovery limits). Used as a non-strategic
    control in scenarios that compare LLM-driven consumers against fixed
    demand.
    """
    ...
    def __init__(
        ...
        """Set up a fixed consumer with a ledger account and per-good inventory slots."""
        self.name = name
