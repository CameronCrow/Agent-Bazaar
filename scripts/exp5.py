#!/usr/bin/env python3
"""
Run Experiment 5 (DLF ablation) — mirrors Experiment 1's DLC sweep but varies firm-side discovery.

Experiment 1 varies discovery-limit-consumers (dlc) ∈ {1,3,5}. This runner holds dlc fixed at 3
(the central Exp1 cell) and sweeps discovery-limit-firms (dlf) ∈ {1,3,5}:
  baseline (n_stab=0) × dlf × seeds 8,16,64;
  stabilizing-firm sweep n_stab ∈ {1,2,3,4,5} × dlf × seeds 8,16,64.
...
Usage: From project root:
  ...
  python scripts/exp5.py --dlf 1 3
  python scripts/exp5.py --n-stab 4 5 --seeds 8
  python scripts/exp5.py --skip-existing --list
"""
import argparse
import subprocess
import sys
...
# Fixed discovery-limit-consumers for this ablation (Exp1's middle grid value).
_FIXED_DLC = 3
...
def build_runs(base: list[str], name_prefix: str) -> list[tuple[str, list[str], dict]]:
    """Construct the full run list: same 54-cell layout as exp1.py, but dlf replaces dlc."""
    runs: list[tuple[str, list[str], dict]] = []
    log_dir_arg = f"logs/{name_prefix}"

    for dlf in (1, 3, 5):
        for seed in (8, 16, 64):
            label = f"{name_prefix}_stab_0_dlf{dlf}_seed{seed}"
            runs.append((
                label,
                ["--name", label, "--log-dir", log_dir_arg,
                 "--discovery-limit-consumers", str(_FIXED_DLC),
                 "--discovery-limit-firms", str(dlf),
                 "--num-stabilizing-firms", "0",
                 "--seed", str(seed)] + base,
                {"dlf": dlf, "n_stab": 0, "seed": seed},
            ))
    ...
    for dlf in (1, 3, 5):
        for n_stab in n_stab_values:
            for seed in (8, 16, 64):
                log_label = f"{name_prefix}_stab_{n_stab}_dlf{dlf}_seed{seed}"
                runs.append((
                    log_label,
                    ["--name", log_label, "--log-dir", log_dir_arg,
                     ...
                     "--seed", str(seed)] + base,
                    {"dlf": dlf, "n_stab": n_stab, "seed": seed},
                ))
...
def filter_runs(
    ...
    for label, argv, meta in runs:
        if run_filter is not None and label not in run_filter:
            continue
        if dlf_filter is not None and meta["dlf"] not in dlf_filter:
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Experiment 5 — discovery-limit-firms (DLF) ablation (THE_CRASH)",
    ...
    parser.add_argument(
        "--dlf", type=int, nargs="+", metavar="N",
        help="Only runs with these --discovery-limit-firms values (e.g. --dlf 1 3).",
    )
    parser.add_argument(
        "--n-stab", type=int, nargs="+", metavar="N", dest="n_stab",
        help="Only runs with these num-stabilizing-firms values.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Skip runs whose directory already exists under logs/exp5_<model>/.",
    )
    ...
    name_prefix = f"exp5_{llm_filesystem_slug(cli.llm)}"
    LOGS_DIR = PROJECT_ROOT / "logs" / name_prefix
    LOGS_DIR.mkdir(parents=True, exist_ok=True)
    SUMMARY_LOG = LOGS_DIR / f"exp5_{TIMESTAMP}.log"
    ...
    if cli.list:
        ...
        for label, _, meta in selected:
            print(
                f"  {label}  [dlc={_FIXED_DLC} dlf={meta['dlf']} "
                f"n_stab={meta['n_stab']} seed={meta['seed']}]"
            )
        return
    ...
    log(f"Experiment 5 (DLF ablation) started. Project root: {PROJECT_ROOT}")
    log(
        f"Selected runs: {len(selected)} / {len(all_runs)}  |  workers: {cli.workers}  |  "
        f"llm: {cli.llm}  |  fixed dlc: {_FIXED_DLC}"
    )
    ...
    log(
        f"Experiment 5 completed. Total wall time: {format_duration(total_wall)}. "
